...
class analitica():
    ...
    def update_data(self, msj):
        ...
        self.publicar("luminosidad",msj_vetor[5])
    ...
    def operaciones(self, sensor):
        ...
        if ("max-{}".format(sensor)=="max-humedad".format(sensor)) and str(df_filtrado.max(skipna = True))>"60":
            publish.single('2/alerthumedad', "Alerta, alto nivel de humedad", hostname='20.49.8.242', client_id='python_client')
            #self.publicar("alerta-humedad".format(sensor), "Alerta, alto nivel de humedad ")

        if ("min-{}".format(sensor)=="min-humedad".format(sensor)) and str(df_filtrado.min(skipna = True))<"40":
            publish.single('2/alerthumedad', "Alerta, bajo nivel de humedad", hostname='20.49.8.242', client_id='python_client')
            #self.publicar("alerta-humedad".format(sensor), "Alerta, bajo nivel de humedad")

        if ("max-{}".format(sensor)=="max-temperatura".format(sensor)) and str(df_filtrado.max(skipna = True))>"31":
            publish.single('2/alerttemperatura', "Alerta, alto nivel de temperatura", hostname='20.49.8.242', client_id='python_client')
            #self.publicar("alerta-temperatura".format(sensor), "Alerta, alto nivel de temperatura ")

        if ("min-{}".format(sensor)=="min-temperatura".format(sensor)) and str(df_filtrado.min(skipna = True))<"28":
            publish.single('2/alerttemperatura', "Alerta, bajo nivel de temperatura", hostname='20.49.8.242', client_id='python_client')
            #self.publicar("alerta-temperatura".format(sensor), "Alerta, bajo nivel de temperatura ")

        if ("min-{}".format(sensor)=="min-luminosidad".format(sensor)) and str(df_filtrado.min(skipna = True))<"23":
            publish.single('2/alertalum', "Lampara encendida, nivel de luminosidad bajo", hostname='20.49.8.242', client_id='python_client')
            #self.publicar("alerta-luminosidad".format(sensor), "Alerta, bajo nivel de luminosidad")


    def analitica_predictiva(self):
        self.regresion("temperatura")
        self.regresion("humedad")
        self.regresion("luminosidad")
    ...
    def regresion(self, sensor):
        ...
        for tiempo, prediccion in zip(nuevos_tiempos, Y_pred):
            ...
            self.publicar("valorpredictiva-{}".format(sensor), "{:.2f}".format(prediccion[0]))
    @staticmethod
